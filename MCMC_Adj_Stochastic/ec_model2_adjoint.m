function [ecs,int_D,params] = ec_model2_adjoint(N,param_vec,ecini,ets,g,f,varargin)
...
%params.Dy=params.Dy/N+ones(g.ny+1,g.nx)+x_Dy.^2+y_Dy.^2;
 ...
 for etc = length(ets):-1:2
    exp_dt = ets(etc)-ets(etc-1);
    nsteps = ceil(exp_dt/maxdt);
    comp_dt = exp_dt/nsteps;
    g.dt = comp_dt;
    if (etc==length(ets))
      ecs(:,:,etc) = exp_dt*f(:,:,etc);
    end  
    ecs(:,:,etc-1) = ecs(:,:,etc);
    for ctc = nsteps:-1:1
      ecs(:,:,etc-1) = advance_ecs(ecs(:,:,etc-1),g,params,f(:,:,etc-1));
    end
